...
def test_untracked_tag_ignore():
    _type = "absolute"
    m.monitor.track("test", _type, _tags, 60, 60)
    m.monitor.record({"another": "tag"}, _dt("2018-12-01T00:00:00Z"), _type, 1)
    assert len(m.monitor.series["test"].data) == 0


def test_backdate_preserve_order():
    _type = "absolute"
    m.monitor.track("test", _type, _tags, 60, 60)
...
def test_type_future_error():
    m.monitor.track("test", "absolute", _tags, 60, 60)
    with pytest.raises(ValueError):
        m.monitor.record(_tags, _now() + timedelta(seconds=1), "absolute", 1)


def test_duplicate_series_error():
    m.monitor.track("test", "absolute", _tags, 60, 60)
    with pytest.raises(ValueError):
        m.monitor.track("test", "absolute", _tags, 60, 60)


def test_invalid_data_series_type():
    with pytest.raises(ValueError):
